...
class NetworkTableInstance final {
  public:
     ...
     * queue is empty (e.g. there are no more events that need to be passed
     * along to callbacks or poll queues) or the timeout expires.
     *
     * @param timeout   timeout, in seconds.  Set to 0 for non-blocking
     * behavior, or a negative value to block indefinitely
     ...
     * connection listener queue is empty (e.g. there are no more events that
     * need to be passed along to callbacks or poll queues) or the timeout
     * expires.
     *
     * @param timeout   timeout, in seconds.  Set to 0 for non-blocking
     * behavior, or a negative value to block indefinitely
    ...
    /**
     * Wait for the incoming RPC call queue to be empty.  This is primarily
     * useful for deterministic testing.  This blocks until either the RPC call
     * queue is empty (e.g. there are no more events that need to be passed
     * along to callbacks or poll queues) or the timeout expires.
     *
     * @param timeout   timeout, in seconds.  Set to 0 for non-blocking
    ...
    /**
     * Starts local-only operation.  Prevents calls to StartServer or
     * StartClient from taking effect.  Has no effect if StartServer or
     * StartClient has already been called.
     */
    void StartLocal();

    /**
     * Stops local-only operation.  StartServer or StartClient can be called
     * after this call to start a server or client.
     */
    void StopLocal();

    /**
     * Starts a server using the specified filename, listening address, and
     * port.
     *
     * @param persist_filename  the name of the persist file to use (UTF-8
     * string, null terminated)
     * @param listen_address    the address to listen on, or null to listen on
     * any address (UTF-8 string, null terminated)
     ...
     * @param port      port to communicate over
     */
    void SetServer(ArrayRef<StringRef> servers,
                   unsigned int port = kDefaultPort);
     ...
     * in the same format from a file at any time on either a client or a
     * server.
     *
     * @param filename  filename
     ...
     */
    const char*
    LoadPersistent(const Twine& filename,
     ...
     */
    const char*
    LoadEntries(const Twine& filename, const Twine& prefix,
                std::function<void(size_t line, const char* msg)> warn);
    ...
    /**
     * Add logger callback function.  By default, log messages are sent to
