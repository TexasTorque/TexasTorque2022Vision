...
class NetworkTableEntry final {
  public:
     ...
     */
    std::vector<int>
    GetBooleanArray(std::initializer_list<int> defaultValue) const;
     ...
     */
    std::vector<double>
    GetDoubleArray(std::initializer_list<double> defaultValue) const;
    ...
    GetStringArray(ArrayRef<std::string> defaultValue) const;
     ...
     */
    std::vector<std::string>
    GetStringArray(std::initializer_list<std::string> defaultValue) const;
    ...
    /**
     * Create a callback-based RPC entry point.  Only valid to use on the
     * server. The callback function will be called when the RPC is called. This
     * function creates RPC version 0 definitions (raw data in and out).
     *
     ...
     */
    NT_EntryListener
    AddListener(std::function<void(const EntryNotification& event)> callback,
