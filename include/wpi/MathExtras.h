...
template<typename T> struct TrailingZerosCounter<T, 8> {
    static std::size_t count(T Val, ZeroBehavior ZB) {
        if (ZB != ZB_Undefined && Val == 0) return 64;
...
template<typename T> T findFirstSet(T Val, ZeroBehavior ZB = ZB_Max) {
    if (ZB == ZB_Max && Val == 0) return (std::numeric_limits<T>::max)();
...
template<unsigned N, unsigned S> constexpr inline bool isShiftedInt(int64_t x) {
    static_assert(
            N > 0,
...
SaturatingMultiply(T X, T Y, bool* ResultOverflowed = nullptr) {
    ...
    Z <<= 1;
    if (X & 1) return SaturatingAdd(Z, Y, ResultOverflowed);
