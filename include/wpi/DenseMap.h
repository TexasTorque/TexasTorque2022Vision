...
class DenseMapBase : public DebugEpochBase {
  ...
  public:
    ...
    ValueT lookup(const_arg_type_t<KeyT> Val) const {
        const BucketT* TheBucket;
        if (LookupBucketFor(Val, TheBucket)) return TheBucket->getSecond();
        return ValueT();
    }
    ...
    std::pair<iterator, bool> try_emplace(KeyT&& Key, Ts&&... Args) {
        ...
        TheBucket = InsertIntoBucket(TheBucket, std::move(Key),
                                     std::forward<Ts>(Args)...);
        return std::make_pair(
                makeIterator(TheBucket, getBucketsEnd(), *this, true), true);
    }
    ...
    bool erase(const KeyT& Val) {
        BucketT* TheBucket;
        if (!LookupBucketFor(Val, TheBucket)) return false; // not in map.
  ...
  protected:
    ...
    /// Returns the number of buckets to allocate to ensure that the DenseMap
    /// can accommodate \p NumEntries without need to grow().
  ...
  private:
    ...
    BucketT* InsertIntoBucketImpl(const KeyT& Key, const LookupKeyT& Lookup,
        ...
        // of the buckets are empty (meaning that many are filled with
        // tombstones), grow the table.
        //
        // The later case is tricky.  For example, if we had one empty bucket
        // with tons of tombstones, failing lookups (e.g. for insertion) would
        // have to probe almost the entire table until it found the empty
        // bucket.  If the table completely filled with tombstones, no lookup
        // would ever succeed, causing infinite loops in lookup.
    ...
    bool LookupBucketFor(const LookupKeyT& Val,
        ...
        while (true) {
            ...
            if (LLVM_LIKELY(
                        KeyInfoT::isEqual(ThisBucket->getFirst(), EmptyKey))) {
...
class SmallDenseMap
  ...
  public:
    ...
    void swap(SmallDenseMap& RHS) {
        ...
        if (Small && RHS.Small) {
            // If we're swapping inline bucket arrays, we have to cope with some
            // of the tricky bits of DenseMap's storage system: the buckets are
            // not fully initialized. Thus we swap every key, but we may have a
            // one-directional move of the value.
    ...
    void copyFrom(const SmallDenseMap& other) {
        ...
        if (other.getNumBuckets() > InlineBuckets) {
            Small = false;
            new (getLargeRep())
                    LargeRep(allocateBuckets(other.getNumBuckets()));
        }
        this->BaseT::copyFrom(other);
    }
  ...
  private:
    ...
    LargeRep allocateBuckets(unsigned Num) {
        ...
        LargeRep Rep = {
                static_cast<BucketT*>(operator new(sizeof(BucketT) * Num)),
                Num};
        return Rep;
    }
};
...
class DenseMapIterator : DebugEpochBase::HandleBase {
  ...
  public:
    ...
    DenseMapIterator(const DenseMapIterator<KeyT, ValueT, KeyInfoT, Bucket,
                                            IsConstSrc>& I)
