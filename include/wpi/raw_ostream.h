...
class raw_ostream {
  private:
    ...
    /// wait for a \see write_impl() call to handle the data which has been put
    /// into this buffer.
  ...
  public:
    ...
    raw_ostream& operator<<(const char* Str) {
        // Inline fast path, particularly for constant strings where a
    ...
    raw_ostream& operator<<(const std::string& Str) {
        // Avoid the fast path, it would only increase code size for a marginal
        // win.
    ...
    /// This function determines if this stream is displayed and supports
    /// colors.
  ...
  private:
    ...
    ///
    /// \param Ptr The start of the data to be written. For buffered streams
    /// this is guaranteed to be the start of the buffer.
    ///
  ...
  private:
    ...
    /// the currently buffered data and resets the buffer to empty.
    void flush_nonempty();
...
class raw_fd_ostream : public raw_pwrite_stream {
  ...
  public:
    ...
    /// this closes the file when the stream is destroyed. If FD is for stdout
    /// or stderr, it will not be closed.
    raw_fd_ostream(int fd, bool shouldClose, bool unbuffered = false);

    ~raw_fd_ostream() override;

    /// Manually flush the stream and close the file. Note that this does not
    /// call fsync.
    void close();
    ...
    /// Flushes the stream and repositions the underlying file descriptor
    ...
    /// Return the value of the flag in this raw_fd_ostream indicating whether
    /// an output error has been encountered. This doesn't implicitly flush any
    /// pending output.  Also, it doesn't guarantee to detect all errors unless
    /// the stream has been closed.
    bool has_error() const {
        return bool(EC);
    }

    /// Set the flag read by has_error() to false. If the error flag is set at
    /// the time when this raw_ostream's destructor is called,
...
class raw_usvector_ostream : public raw_pwrite_stream {
  ...
  public:
    ...
    ArrayRef<uint8_t> array() {
        return ArrayRef<uint8_t>(OS.data(), OS.size());
    }
};
