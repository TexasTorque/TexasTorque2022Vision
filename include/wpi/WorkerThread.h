...
template<typename R> struct WorkerThreadAsync {
...
template<> struct WorkerThreadAsync<void> {
...
template<typename R, typename... T> struct WorkerThreadRequest {
    ...
    WorkerThreadRequest(uint64_t promiseId_, WorkFunction work_,
                        std::tuple<T...> params_)
        : promiseId(promiseId_), work(std::move(work_)),
          params(std::move(params_)) {
    }
    WorkerThreadRequest(WorkFunction work_, AfterWorkFunction afterWork_,
                        std::tuple<T...> params_)
        : promiseId(0), work(std::move(work_)),
          afterWork(std::move(afterWork_)), params(std::move(params_)) {
    }
...
template<typename R, typename... T> void WorkerThreadThread<R, T...>::Main() {
...
template<typename R, typename... T> class WorkerThread<R(T...)> final {
  ...
  public:
     ...
     */
    template<typename... U> future<R> QueueWork(WorkFunction work, U&&... u) {
        if (auto thr = m_owner.GetThread()) {
            // create the future
     ...
     */
    template<typename... U>
    void QueueWorkThen(WorkFunction work, AfterWorkFunction afterWork,
                       U&&... u) {
        if (auto thr = m_owner.GetThread()) {
            // add the parameters to the input queue
            thr->m_requests.emplace_back(
