...
class StringRef {
  ...
  private:
    ...
    static int compareMemory(const char* Lhs, const char* Rhs,
  ...
  public:
    ...
    int compare(StringRef RHS) const noexcept {
        ...
        if (int Res = compareMemory(Data, RHS.Data,
    ...
    size_t find(char C, size_t From = 0) const noexcept {
        ...
        if (FindBegin < Length) { // Avoid calling memchr with nullptr.
            ...
            if (const void* P =
                        ::memchr(Data + FindBegin, C, Length - FindBegin))
    ...
    LLVM_NODISCARD
    LLVM_ATTRIBUTE_ALWAYS_INLINE
    bool contains(StringRef Other) const noexcept {
    ...
    LLVM_NODISCARD
    LLVM_ATTRIBUTE_ALWAYS_INLINE
    bool contains_lower(char C) const noexcept {
    ...
    std::pair<StringRef, StringRef> split(StringRef Separator) const {
        ...
        return std::make_pair(slice(0, Idx),
                              slice(Idx + Separator.size(), npos));
    }
