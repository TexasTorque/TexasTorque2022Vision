...
template<class T> inline void deleter(T* Ptr) {
    delete Ptr;
}
...
template<typename ContainerTy>
auto adl_begin(ContainerTy&& container) -> decltype(adl_detail::adl_begin(
        std::forward<ContainerTy>(container))) {
...
class filter_iterator_base
    : public iterator_adaptor_base<
              ...
              typename std::common_type<
                      IterTag, typename std::iterator_traits<WrappedIteratorT>::
  ...
  protected:
    ...
    void findNextValid() {
        while (this->I != End && !Pred(*this->I)) BaseT::operator++();
    }

    // Construct the iterator. The begin iterator needs to know where the end
    // is, so that it can properly stop when it gets there. The end iterator
    // only needs the predicate to support bidirectional iteration.
...
class filter_iterator_impl<WrappedIteratorT, PredicateT,
    ...
    using BaseT = filter_iterator_base<WrappedIteratorT, PredicateT,
                                       std::bidirectional_iterator_tag>;
    void findPrevValid() {
        while (!this->Pred(*this->I)) BaseT::operator--();
    }
...
using zip_traits = iterator_facade_base<
        ...
        // instance, the second field's difference_type were non-numeric while
        // the first is.
        typename ZipTupleType<Iters...>::type*,
        typename ZipTupleType<Iters...>::type>;
...
class zip_longest_iterator
    : public iterator_facade_base<
              ...
              typename std::common_type<std::forward_iterator_tag,
                                        typename std::iterator_traits<Iters>::
                                                iterator_category...>::type,
  ...
  private:
    ...
    template<size_t... Ns> value_type deref(std::index_sequence<Ns...>) const {
        return value_type(deref_or_none(std::get<Ns>(iterators),
                                        std::get<Ns>(end_iterators))...);
    }

    template<size_t... Ns>
    decltype(iterators) tup_inc(std::index_sequence<Ns...>) const {
        return std::tuple<Iters...>(next_or_end(
                std::get<Ns>(iterators), std::get<Ns>(end_iterators))...);
    }
...
class concat_iterator
    ...
    ///
    /// Note that something like iterator_range seems nice at first here, but
    /// the range properties are of little benefit and end up getting in the way
    ...
    template<size_t... Ns> void increment(std::index_sequence<Ns...>) {
        ...
        for (auto& IncrementHelperFn : IncrementHelperFns)
            if ((this->*IncrementHelperFn)()) return;
    ...
    /// Finds the first non-end iterator, dereferences, and returns the
    /// resulting reference.
    ///
    /// It is an error to call this with all iterators at the end.
    template<size_t... Ns> ValueT& get(std::index_sequence<Ns...>) const {
        ...
        for (auto& GetHelperFn : GetHelperFns)
            if (ValueT* P = (this->*GetHelperFn)()) return *P;
...
template<typename T, typename U, typename... Ts> struct is_one_of<T, U, Ts...> {
...
auto count(R&& Range, const E& Element) -> typename std::iterator_traits<
        decltype(adl_begin(Range))>::difference_type {
...
bool hasNItems(IterTy&& Begin, IterTy&& End, unsigned N,
               typename std::enable_if<
                       ...
                       void>::type* = nullptr) {
    for (; N; --N, ++Begin)
        if (Begin == End) return false; // Too few.
    return Begin == End;
}
