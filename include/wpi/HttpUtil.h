...
class HttpPath {
  public:
    ...
    bool equals(ArrayRef<StringRef> match) const {
        return equals(0, match);
    }
    bool equals(StringRef match) const {
    ...
    bool startswith(size_t start,
                    std::initializer_list<StringRef> match) const {
...
class HttpRequest {
  ...
  private:
    ...
    template<typename T> void SetPath(StringRef path_, const T& params);

    template<typename T> static StringRef GetFirst(const T& elem) {
        return elem.first;
    }
    template<typename T>
    ...
    template<typename T> static StringRef GetSecond(const T& elem) {
        return elem.second;
    }
};
...
class HttpMultipartScanner {
  public:
    explicit HttpMultipartScanner(StringRef boundary,
    ...
    // Execute the scanner.  Will automatically call Reset() on entry if
    // IsDone() is true.
    // @param in input data
