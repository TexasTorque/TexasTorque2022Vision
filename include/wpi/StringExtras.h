...
inline StringRef toStringRef(ArrayRef<uint8_t> Input) {
    return StringRef(reinterpret_cast<const char*>(Input.begin()),
                     Input.size());
}
...
inline bool isASCII(wpi::StringRef S) {
    for (char C : S)
        if (LLVM_UNLIKELY(!isASCII(C))) return false;
    return true;
}
...
inline uint8_t hexFromNibbles(char MSB, char LSB) {
    ...
    assert(U1 != (std::numeric_limits<unsigned>::max)() &&
           U2 != (std::numeric_limits<unsigned>::max)());
...
inline std::string join_impl(IteratorT Begin, IteratorT End,
    ...
    for (IteratorT I = Begin; I != End; ++I) Len += (*Begin).size();
    S.reserve(Len);
    S += (*Begin);
    while (++Begin != End) {
        S += Separator;
...
inline std::string join_items(Sep Separator, Args&&... Items) {
    std::string Result;
    if (sizeof...(Items) == 0) return Result;
