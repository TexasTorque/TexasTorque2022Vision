...
inline value_type readAtBitAlignment(const void* memory, uint64_t startBit) {
    ...
    if (startBit == 0)
        ...
        lowerVal &=
                ((std::make_unsigned_t<value_type>)1 << numBitsFirstVal) - 1;

        // Get the bits from the upper value.
        std::make_unsigned_t<value_type> upperVal =
                val[1] &
                (((std::make_unsigned_t<value_type>)1 << startBit) - 1);
...
inline void writeAtBitAlignment(void* memory, value_type value,
    ...
    if (startBit == 0)
        ...
        if (startBit > 0) {
            // Mask off the upper bits in the new value that are not going to go
            // into the lower value. This avoids a left shift of a negative
            // value, which is undefined behavior.
            lowerVal &=
                    (((std::make_unsigned_t<value_type>)1 << numBitsFirstVal) -
                     1);
