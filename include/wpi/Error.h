...
class LLVM_NODISCARD Error {
  ...
  public:
    ...
    /// Move-assign an error value. The current error must represent success,
    /// you you cannot overwrite an unhandled error. The current error is then
    ...
    /// and false if it is in an accept state. If the error is in a Success
    /// state it will be considered checked.
  ...
  public:
    ...
    Expected(OtherT&& Val,
             typename std::enable_if<
                     std::is_convertible<OtherT, T>::value>::type* = nullptr)
        : HasError(false) {
    ...
    /// Move construct an Expected<T> value from an Expected<OtherT>, where
    /// OtherT must be convertible to T.
    ...
    /// only be safely destructed. No further calls (beside the destructor)
    /// should be made on the Expected<T> vaule.
    Error takeError() {
  ...
  private:
    ...
    template<class OtherT> void moveConstruct(Expected<OtherT>&& Other) {
        ...
        if (!HasError)
            ...
            new (getErrorStorage())
                    error_type(std::move(*Other.getErrorStorage()));
    }
...
inline void cantFail(Error Err, const char* Msg = nullptr) {
    if (Err) {
        if (!Msg) Msg = "Failure value returned from cantFail wrapped call";
        wpi_unreachable(Msg);
    }
}
...
template<typename... HandlerTs> Error handleErrors(Error E, HandlerTs&&... Hs) {
    if (!E) return Error::success();
    ...
    if (Payload->isA<ErrorList>()) {
        ...
        for (auto& P : List.Payloads)
            R = ErrorList::join(
                    std::move(R),
                    handleErrorImpl(std::move(P),
                                    std::forward<HandlerTs>(Hs)...));
        return R;
    }
...
inline bool errorToBool(Error Err) {
    ...
    if (IsError) consumeError(std::move(Err));
    return IsError;
}
...
template<typename T> class ExpectedAsOutParameter {
  public:
    ExpectedAsOutParameter(Expected<T>* ValOrErr) : ValOrErr(ValOrErr) {
        if (ValOrErr) (void)!!*ValOrErr;
    }

    ~ExpectedAsOutParameter() {
        if (ValOrErr) ValOrErr->setUnchecked();
    }
...
template<typename T> Expected<T> errorOrToExpected(ErrorOr<T>&& EO) {
    if (auto EC = EO.getError()) return errorCodeToError(EC);
    return std::move(*EO);
}

/// Convert an Expected<T> to an ErrorOr<T>.
template<typename T> ErrorOr<T> expectedToErrorOr(Expected<T>&& E) {
    if (auto Err = E.takeError()) return errorToErrorCode(std::move(Err));
    return std::move(*E);
}
...
class FileError final : public ErrorInfo<FileError> {
  ...
  private:
    ...
    static Error build(std::string F, Error E) {
        return Error(
                std::unique_ptr<FileError>(new FileError(F, E.takePayload())));
    }
