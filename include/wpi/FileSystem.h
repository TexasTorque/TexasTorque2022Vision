...
class basic_file_status {
  ...
  public:
    ...
    ///
    /// Also see comments on \c getLastModificationTime() related to the
    ...
    /// resolution can differ across platforms and even across mountpoints on
    /// the same machine.
...
#ifndef _WIN32
    file_status(file_type Type, perms Perms, dev_t Dev, nlink_t Links,
                ino_t Ino, time_t ATime, uint32_t ATimeNSec, time_t MTime,
...
inline bool is_regular_file(const Twine& Path) {
    bool Result;
    if (is_regular_file(Path, Result)) return false;
    return Result;
}
...
inline bool is_symlink_file(const Twine& Path) {
    bool Result;
    if (is_symlink_file(Path, Result)) return false;
    return Result;
}
...
class directory_entry {
  ...
  public:
    ...
    file_type type() const {
        if (Type != file_type::type_unknown) return Type;
        auto S = status();
...
class directory_iterator {
  ...
  public:
    ...
    bool operator==(const directory_iterator& RHS) const {
        ...
        if (!RHS.State) return State->CurrentEntry == directory_entry();
        if (!State) return RHS.State->CurrentEntry == directory_entry();
...
class recursive_directory_iterator {
  ...
  public:
    ...
    explicit recursive_directory_iterator(const Twine& path,
        ...
        if (State->Stack.top() == directory_iterator()) State.reset();
    }
