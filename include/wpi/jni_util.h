...
template<typename T> class JGlobal {
  public:
    JGlobal() = default;
...
template<typename T> class JLocal {
  public:
...
template<typename C, typename T> class JArrayRefInner {};

// Specialization of JArrayRefBase to provide StringRef conversion.
template<typename C> class JArrayRefInner<C, jbyte> {
  public:
    operator StringRef() const {
        return str();
    }
...
class JArrayRefBase : public JArrayRefInner<JArrayRefBase<T>, T> {
  public:
    ...
    JArrayRefBase(JArrayRefBase&& oth)
        : m_env(oth.m_env), m_jarr(oth.m_jarr), m_size(oth.m_size),
      ...
      public:                                                                  \
        J##F##ArrayRef(JNIEnv* env, jobject bb, int len)                       \
            : detail::JArrayRefBase<T>(                                        \
                      env,                                                     \
                      static_cast<T*>(bb ? env->GetDirectBufferAddress(bb)     \
                                         : nullptr),                           \
                      len) {                                                   \
            if (!bb) {                                                         \
        ...
        ~J##F##ArrayRef() {                                                    \
            if (m_jarr && m_elements) {                                        \
                m_env->Release##F##ArrayElements(                              \
                        static_cast<T##Array>(m_jarr), m_elements, JNI_ABORT); \
            }                                                                  \
        }                                                                      \
    };                                                                         \
                                                                               \
    class CriticalJ##F##ArrayRef : public detail::JArrayRefBase<T> {           \
      public:                                                                  \
        ...
        ~CriticalJ##F##ArrayRef() {                                            \
            if (m_jarr && m_elements) {                                        \
                m_env->ReleasePrimitiveArrayCritical(m_jarr, m_elements,       \
                                                     JNI_ABORT);               \
            }                                                                  \
        }                                                                      \
    };
...
struct ConvertIntArray {
    static jintArray ToJava(JNIEnv* env, ArrayRef<T> arr) {
        ...
        jint* elements = static_cast<jint*>(
                env->GetPrimitiveArrayCritical(jarr, nullptr));
...
template<typename T> struct ConvertIntArray<T, true> {
    static jintArray ToJava(JNIEnv* env, ArrayRef<T> arr) {
        jintArray jarr = env->NewIntArray(arr.size());
        if (!jarr) { return nullptr; }
...
inline jbooleanArray MakeJBooleanArray(JNIEnv* env, ArrayRef<int> arr) {
    ...
    if (!jarr) { return nullptr; }
    jboolean* elements = static_cast<jboolean*>(
            env->GetPrimitiveArrayCritical(jarr, nullptr));
    if (!elements) { return nullptr; }
    for (size_t i = 0; i < arr.size(); ++i) {
...
template<typename T> class JCallbackThread : public SafeThread {
  public:
    void Main() override;
...
template<typename T> void JCallbackThread<T>::Main() {
    JNIEnv* env;
    ...
    for (jsize i = 0; i < stackTraceLength; i++) {
        ...
        JLocal<jstring> stackElementString(
                env, static_cast<jstring>(env->CallObjectMethod(
                             curStackTraceElement, toStringId)));
...
class JException : public JClass {
  public:
    ...
    void Throw(JNIEnv* env, StringRef msg) {
