...
template<typename Callback> class CallbackListenerData {
  public:
...
template<typename Derived, typename TUserInfo,
         typename TListenerData = CallbackListenerData<
                 std::function<void(const TUserInfo& info)>>,
        ...
        while (!m_queue.empty()) {
            ...
            if (item.first != UINT_MAX) {
                if (item.first < m_listeners.size()) {
                    ...
                    if (listener && static_cast<Derived*>(this)->Matches(
                                            listener, item.second)) {
                        static_cast<Derived*>(this)->SetListener(&item.second,
                                                                 item.first);
                        if (listener.callback) {
                            lock.unlock();
                            static_cast<Derived*>(this)->DoCallback(
                                    listener.callback, item.second);
                            lock.lock();
                        } else if (listener.poller_uid != UINT_MAX) {
                            SendPoller(listener.poller_uid,
                ...
                for (size_t i = 0; i < m_listeners.size(); ++i) {
                    ...
                    if (!static_cast<Derived*>(this)->Matches(listener,
                                                              item.second)) {
                        continue;
                    }
                    static_cast<Derived*>(this)->SetListener(
                            &item.second, static_cast<unsigned>(i));
                    if (listener.callback) {
                        lock.unlock();
                        static_cast<Derived*>(this)->DoCallback(
                                listener.callback, item.second);
...
// Derived must define the following functions:
//   void Start();
template<typename Derived, typename Thread> class CallbackManager {
  ...
  public:
    ...
    bool WaitForQueue(double timeout) {
        ...
        while (!thr->m_queue.empty()) {
            ...
            if (timeout < 0) {
                ...
                auto cond_timed_out =
                        thr->m_queue_empty.wait_until(lock, timeout_time);
    ...
    std::vector<typename Thread::UserInfo>
    Poll(unsigned int poller_uid, double timeout, bool* timed_out) {
        ...
        *timed_out = false;
        while (poller->poll_queue.empty()) {
            ...
            if (poller->canceling) {
                ...
                // function for any particular poller, but that's the intended
                // use.
            ...
            if (timeout < 0) {
                ...
                auto cond_timed_out =
                        poller->poll_cond.wait_until(lock, timeout_time);
  ...
  protected:
    template<typename... Args> void DoStart(Args&&... args) {
        m_owner.Start(std::forward<Args>(args)...);
    }

    template<typename... Args> unsigned int DoAdd(Args&&... args) {
        static_cast<Derived*>(this)->Start();
        auto thr = m_owner.GetThread();
    ...
    void Send(unsigned int only_listener, Args&&... args) {
        ...
        thr->m_queue.emplace(
                std::piecewise_construct, std::make_tuple(only_listener),
