...
class unique_function<ReturnT(ParamTs...)> {
    ...
    // and all other types to l-value reference types. We use this to compute
    // the types used in our erased call utility to minimize copies and moves
    // unless doing so would force things unnecessarily into memory.
    //
    ...
    // The type of the erased function pointer we use as a callback to dispatch
    // to the stored callable when it is trivial to move and destroy.
    ...
    // pointer in a struct or a pointer to a static struct of the call, move,
    // and destroy pointers.
    ...
    union StorageUnionT {
        ...
        // and the size. This is enough to deallocate the memory.
        struct OutOfLineStorageT {
            void* StoragePtr;
            size_t Size;
            size_t Alignment;
        } OutOfLineStorage;
        static_assert(sizeof(OutOfLineStorageT) <= InlineStorageSize,
                      "Should always use all of the out-of-line storage for "
                      "inline storage!");
    ...
    template<typename CallableT>
    static void MoveImpl(void* LHSCallableAddr,
                         void* RHSCallableAddr) noexcept {
        new (LHSCallableAddr) CallableT(
                std::move(*reinterpret_cast<CallableT*>(RHSCallableAddr)));
    }
  ...
  public:
    ...
    unique_function& operator=(unique_function&& RHS) noexcept {
        ...
        // Because we don't try to provide any exception safety guarantees we
        // can implement move assignment very simply by first destroying the
        // current object and then move-constructing over top of it.
    ...
    unique_function(CallableT Callable,
        ...
        // trivially moved and trivially destroyed so that we don't have to
        // store type erased callbacks for those operations.
        //
        // FIXME: We should use constexpr if here and below to avoid
        // instantiating the non-trivial static objects when unnecessary. While
        // the linker should remove them, it is still wasteful.
        if (std::is_trivially_move_constructible<CallableT>::value &&
            std::is_trivially_destructible<CallableT>::value) {
            // We need to create a nicely aligned object. We use a static
        ...
        // different type erased behaviors needed. Create a static instance of
        // the struct type here and then use a pointer to that.
        static NonTrivialCallbacks Callbacks = {&CallImpl<CallableT>,
                                                &MoveImpl<CallableT>,
