...
class SmallVectorBase {
  ...
  public:
    ...
    ///
    /// Clients can use this in conjunction with capacity() to write past the
    /// end of the buffer when they know that more elements are available, and
    /// only update the size later. This avoids the cost of value initializing
...
class SmallVectorTemplateBase : public SmallVectorTemplateCommon<T> {
  ...
  public:
    void push_back(const T& Elt) {
        if (LLVM_UNLIKELY(this->size() >= this->capacity())) this->grow();
...
void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize) {
    ...
    NewCapacity =
            (std::min)((std::max)(NewCapacity, MinSize), size_t(UINT32_MAX));
...
template<typename T> class SmallVectorImpl : public SmallVectorTemplateBase<T> {
  ...
  public:
    ...
    iterator insert(iterator I, T&& Elt) {
        ...
        // the reference.
        T* EltPtr = &Elt;
        if (I <= EltPtr && EltPtr < this->end()) ++EltPtr;
...
template<typename T>
inline void swap(wpi::SmallVectorImpl<T>& LHS, wpi::SmallVectorImpl<T>& RHS) {
    LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template<typename T, unsigned N>
inline void swap(wpi::SmallVector<T, N>& LHS, wpi::SmallVector<T, N>& RHS) {
    LHS.swap(RHS);
}
