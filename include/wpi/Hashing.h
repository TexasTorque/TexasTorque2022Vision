...
struct hash_state {
    ...
    uint64_t finalize(size_t length) {
        return hash_16_bytes(hash_16_bytes(h3, h5) + shift_mix(h1) * k1 + h2,
                             hash_16_bytes(h4, h6) + shift_mix(length) * k1 +
                                     h0);
    }
};
  ...
  public:
    ...
    char* combine_data(size_t& length, char* buffer_ptr, char* buffer_end,
        ...
        if (!store_and_advance(buffer_ptr, buffer_end, data)) {
            // Check for skew which prevents the buffer from being packed, and
            // do a partial store into the buffer to fill it. This is only a
            // concern with the variadic combine because that formation can have
            // varying argument types.
            size_t partial_store_size = buffer_end - buffer_ptr;
            memcpy(buffer_ptr, &data, partial_store_size);

            // If the store fails, our buffer is full and ready to hash. We have
            // to either initialize the hash state (on the first full buffer) or
            // mix this buffer into the existing hash state. Length tracks the
            // *hashed* length, not the buffered length.
            if (length == 0) {
    ...
    hash_code combine(size_t length, char* buffer_ptr, char* buffer_end) {
        ...
        // we'll use the optimized short hashing routine and skip state
        // entirely.
        if (length == 0)
            return static_cast<size_t>(
                    hash_short(buffer, buffer_ptr - buffer, seed));

        // Mix the final buffer, rotating it if we did a partial fill in order
        // to simulate doing a mix of the last 64-bytes. That is how the
        // algorithm works when we have a contiguous byte sequence, and we want
