...
template<typename T> class PromiseFactory;
...
class PromiseFactoryBase {
  public:
    ...
    bool
    WaitUntil(std::unique_lock<wpi::mutex>& lock,
...
template<typename To, typename From> struct FutureThen {
    template<typename F>
    static future<To> Create(PromiseFactory<From>& fromFactory,
                             uint64_t request, PromiseFactory<To>& factory,
                             F&& func);
};

template<typename From> struct FutureThen<void, From> {
    template<typename F>
...
template<typename To> struct FutureThen<To, void> {
    template<typename F>
    static future<To> Create(PromiseFactory<void>& fromFactory,
                             uint64_t request, PromiseFactory<To>& factory,
                             F&& func);
};

template<> struct FutureThen<void, void> {
    template<typename F>
  ...
  private:
    struct Then {
        Then(uint64_t request_, uint64_t outRequest_, ThenFunction func_)
            : request(request_), outRequest(outRequest_),
              func(std::move(func_)) {
        }
        uint64_t request;
        uint64_t outRequest;
...
template<typename T> class future final {
  ...
  public:
     ...
     * Can only be called once.  The future will be marked invalid after the
     * call.
     *
    ...
    future<R> then(PromiseFactory<R>& factory, F&& func) {
        if (m_promises) {
            ...
            m_promises = nullptr;
            return detail::FutureThen<R, T>::Create(*promises, m_request,
                                                    factory, func);
        } else {
            return future<R>();
        }
     ...
     * A default-constructed future or one where get() has been called is
     * invalid.
     *
    ...
    wait_until(const std::chrono::time_point<Clock, Duration>& timeout_time)
            const {
        return m_promises &&
               m_promises->WaitResultUntil(m_request, timeout_time);
    }
 ...
 */
template<> class future<void> final {
  ...
  public:
    ...
    future<R> then(PromiseFactory<R>& factory, F&& func) {
        if (m_promises) {
            auto promises = m_promises;
            m_promises = nullptr;
            return detail::FutureThen<R, void>::Create(*promises, m_request,
                                                       factory, func);
        } else {
            return future<R>();
        }
...
template<typename T> class promise final {
  ...
  public:
    ...
    future<T> get_future() noexcept {
        return future<T>(m_promises, m_request);
    }
 ...
 */
template<typename T> inline future<T> make_ready_future(T&& value) {
...
template<typename T> future<T> PromiseFactory<T>::MakeReadyFuture(T&& value) {
...
template<typename T> T PromiseFactory<T>::GetResult(uint64_t request) {
    // wait for response
    std::unique_lock lock(GetResultMutex());
    while (IsActive()) {
...
template<typename T> void PromiseFactory<T>::WaitResult(uint64_t request) {
    // wait for response
    std::unique_lock lock(GetResultMutex());
    while (IsActive()) {
        // Did we get a response to *our* request?
...
template<typename T> PromiseFactory<T>& PromiseFactory<T>::GetInstance() {
...
template<typename F>
future<To>
detail::FutureThen<To, From>::Create(PromiseFactory<From>& fromFactory,
                                     uint64_t request,
...
template<typename F>
future<To>
detail::FutureThen<To, void>::Create(PromiseFactory<void>& fromFactory,
                                     uint64_t request,
                                     PromiseFactory<To>& factory, F&& func) {
    uint64_t req = factory.CreateRequest();
