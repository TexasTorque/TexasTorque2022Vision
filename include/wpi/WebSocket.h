...
class WebSocket : public std::enable_shared_from_this<WebSocket> {
  ...
  public:
     ...
     */
    static std::shared_ptr<WebSocket>
    CreateClient(uv::Stream& stream, const Twine& uri, const Twine& host,
    ...
    /**
     * Starts a server connection by performing the initial server side
     * handshake. This should be called after the HTTP headers have been
     * received. An open event is emitted when the handshake completes. This
     * sets the stream user data to the websocket.
     * @param stream Connection stream
     ...
     */
    static std::shared_ptr<WebSocket>
    CreateServer(uv::Stream& stream, StringRef key, StringRef version,
    ...
    /**
     * Set the maximum message size.  Default is 128 KB.  If configured to
     * combine fragments this maximum applies to the entire message (all
     * combined fragments).
     ...
     * combine.  If fragmented frames are combined, the text and binary
     * callbacks will always have the second parameter (fin) set to true.
     * @param combine True if fragmented frames should be combined.
     */
    void SetCombineFragments(bool combine) {
        m_combineFragments = combine;
    }
     ...
     */
    void SendText(ArrayRef<uv::Buffer> data,
                  std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                          callback) {
     ...
     * @param callback Callback which is invoked when the write completes.
     */
    void SendBinary(ArrayRef<uv::Buffer> data,
                    std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                            callback) {
     ...
     */
    void
    SendTextFragment(ArrayRef<uv::Buffer> data,
                     std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                             callback) {
     ...
     */
    void
    SendFragment(ArrayRef<uv::Buffer> data, bool fin,
                 std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                         callback) {
     ...
     *                 write completes.
     */
    void SendPing(ArrayRef<uv::Buffer> data,
                  std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                          callback) {
     ...
     *                 write completes.
     */
    void SendPong(ArrayRef<uv::Buffer> data,
                  std::function<void(MutableArrayRef<uv::Buffer>, uv::Error)>
                          callback) {
     ...
     */
    template<typename T = void> std::shared_ptr<T> GetData() const {
    ...
    /**
     * Open event.  Emitted when the connection is open and ready to
     * communicate. The parameter is the selected subprotocol.
     */
    sig::Signal<StringRef> open;

    /**
     * Close event.  Emitted when the connection is closed.  The first parameter
     * is a numeric value indicating the status code explaining why the
     * connection has been closed.  The second parameter is a human-readable
     * string explaining the reason why the connection has been closed.
     */
  ...
  private:
    ...
    void
    Send(uint8_t opcode, ArrayRef<uv::Buffer> data,
