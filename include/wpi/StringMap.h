...
class StringMapImpl {
  ...
  protected:
    ...
    /// end up in.  If it already exists as a key in the map, the Item pointer
    /// for the specified bucket will be non-null.  Otherwise, it will be null.
    /// In either case, the FullHashValue field of the bucket will be set to the
    /// hash value of the string.
    unsigned LookupBucketFor(StringRef Key);

    /// FindKey - Look up the bucket that contains the specified key. If it
    /// exists in the map, return the bucket number of the key.  Otherwise
    /// return -1. This does not modify the map.
    int FindKey(StringRef Key) const;

    /// RemoveKey - Remove the specified StringMapEntry from the table, but do
    /// not delete it.  This aborts if the value isn't in the table.
...
template<typename ValueTy> class StringMapEntry : public StringMapEntryBase {
  public:
    ...
    /// getKeyData - Return the start of the string data that is the key for
    /// this value.  The string data is always stored immediately after the
    ...
    static StringMapEntry* Create(StringRef Key, InitTy&&... InitVals) {
        ...
        if (KeyLength > 0) memcpy(StrBuffer, Key.data(), KeyLength);
    ...
    /// GetStringMapEntryFromKeyData - Given key data that is known to be
    /// embedded into a StringMapEntry, return the StringMapEntry itself.
    static StringMapEntry& GetStringMapEntryFromKeyData(const char* KeyData) {
        char* Ptr =
                const_cast<char*>(KeyData) - sizeof(StringMapEntry<ValueTy>);
...
template<typename ValueTy> class StringMap : public StringMapImpl {
  public:
    ...
    StringMap(StringMap&& RHS) : StringMapImpl(std::move(RHS)) {
    }
    ...
    StringMap(const StringMap& RHS)
        ...
        // tombstones included. We could, instead, have re-probed for each key
        // to instantiate this new object without any tombstone buckets. The
        // assumption here is that items are rarely deleted from most
        // StringMaps, and so tombstones are rare, so the cost of re-probing for
        // all inputs is not worthwhile.
    }
    ...
    /// isn't already in the map. The bool component of the returned pair is
    /// true if and only if the insertion takes place, and the iterator
    ...
    /// Emplace a new element for the specified key into the map if the key
    /// isn't already in the map. The bool component of the returned pair is
    /// true if and only if the insertion takes place, and the iterator
    /// component of the pair points to the element with key equivalent to the
    /// key of the pair.
    ...
    void clear() {
        ...
        // tombstone), which is safe because we're removing all elements.
        for (unsigned I = 0, E = NumBuckets; I != E; ++I) {
            StringMapEntryBase*& Bucket = TheTable[I];
            if (Bucket && Bucket != getTombstoneVal()) {
                static_cast<MapEntryTy*>(Bucket)->Destroy();
...
class StringMapIterator : public StringMapIterBase<StringMapIterator<ValueTy>,
    ...
    using base = StringMapIterBase<StringMapIterator<ValueTy>,
                                   StringMapEntry<ValueTy>>;
