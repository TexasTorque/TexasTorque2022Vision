...
class VisionRunnerBase {
  public:
     ...
     * notify it that the pipeline ran. This must be run in a dedicated thread,
     * and cannot be used in the main robot thread because it will freeze the
     * robot program.
     *
     * <p>This method is exposed to allow teams to add additional functionality
     * or have their own ways to run the pipeline. Most teams, however, should
     * just use {@link #runForever} in its own thread using a std::thread.</p>
     */
    void RunOnce();
