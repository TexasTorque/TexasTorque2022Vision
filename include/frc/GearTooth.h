...
class GearTooth : public Counter {
  public:
     ...
     *
     * @param channel            The DIO channel that the sensor is connected
     * to. 0-9 are on-board, 10-25 are on the MXP.
     * @param directionSensitive True to enable the pulse length decoding in
     *                           hardware to specify count direction.
     */
    WPI_DEPRECATED("The only sensor this works with is no longer available and "
                   "no teams use "
     ...
     * This should be used when sharing digital inputs.
     *
     * @param source             A reference to the existing DigitalSource
     * object (such as a DigitalInput)
     * @param directionSensitive True to enable the pulse length decoding in
     *                           hardware to specify count direction.
     */
    WPI_DEPRECATED("The only sensor this works with is no longer available and "
