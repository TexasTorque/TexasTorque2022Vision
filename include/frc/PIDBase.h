...
class PIDBase : public PIDInterface,
  ...
  public:
     ...
     *
     * Rather then using the max and min input range as constraints, it
     * considers them to be the same point and automatically calculates the
     ...
     * the setpoint. Ideally it should be based on being within the tolerance
  ...
  protected:
     ...
     * output. This should only be called by the Notifier.
     */
     ...
     * no synchronization because the PIDBase class only calls it in
    ...
    /**
     * Wraps error around for continuous inputs. The original error is returned
     * if continuous mode is disabled. This is an unsynchronized function.
     *
