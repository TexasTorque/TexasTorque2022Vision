...
class TimedRobot : public IterativeRobotBase, public ErrorBase {
  public:
     ...
     *
     * This is scheduled on TimedRobot's Notifier, so TimedRobot and the
     * callback run synchronously. Interactions between them are thread-safe.
     *
     * @param callback The callback to run.
     * @param period   The period at which to run the callback.
     * @param offset   The offset from the common starting time. This is useful
     *                 for scheduling a callback in a different timeslot
     * relative to TimedRobot.
     */
  ...
  private:
    class Callback {
      public:
         ...
         * @param startTime The common starting point for all callback
         * scheduling.
         * @param period    The period at which to run the callback.
        ...
        Callback(std::function<void()> func, units::second_t startTime,
            ...
            : func{std::move(func)}, period{period},
              expirationTime{
                      startTime + offset +
                      units::math::floor(
                              (frc2::Timer::GetFPGATimestamp() - startTime) /
                              period) *
                              period +
                      period} {
        }
