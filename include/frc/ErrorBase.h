...
#define wpi_setImaqErrorWithContext(code, context)                             \
    do {                                                                       \
        if ((code) != 0)                                                       \
            this->SetImaqError((code), (context), __FILE__, __FUNCTION__,      \
                               __LINE__);                                      \
    } while (0)
#define wpi_setErrorWithContext(code, context)                                 \
    do {                                                                       \
        if ((code) != 0)                                                       \
            this->SetError((code), (context), __FILE__, __FUNCTION__,          \
                           __LINE__);                                          \
    } while (0)
#define wpi_setErrorWithContextRange(code, min, max, req, context)             \
    do {                                                                       \
        if ((code) != 0)                                                       \
            this->SetErrorRange((code), (min), (max), (req), (context),        \
    ...
    do {                                                                       \
        if ((code) != 0)                                                       \
            this->SetErrorRange((code), (min), (max), (req),                   \
                                HAL_GetErrorMessage(code), __FILE__,           \
                                __FUNCTION__, __LINE__);                       \
    } while (0)
...
#define wpi_setStaticErrorWithContext(object, code, context)                   \
    do {                                                                       \
        if ((code) != 0)                                                       \
            object->SetError((code), (context), __FILE__, __FUNCTION__,        \
                             __LINE__);                                        \
    } while (0)
...
#define wpi_setGlobalHALError(code)                                            \
    do {                                                                       \
        if ((code) != 0)                                                       \
            ::frc::ErrorBase::SetGlobalError(                                  \
                    (code), HAL_GetErrorMessage(code), __FILE__, __FUNCTION__, \
                    __LINE__);                                                 \
    } while (0)
...
class ErrorBase {
  ...
  public:
    ...
    /**
     * @brief Set the current error information associated from the nivision
     * Imaq API.
     *
