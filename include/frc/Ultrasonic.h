...
class Ultrasonic : public ErrorBase,
  ...
  public:
     ...
     *
     * @param pingChannel The digital output object that starts the sensor doing
     * a ping. Requires a 10uS pulse to start.
     * @param echoChannel The digital input object that times the return pulse
     ...
     *
     * The ranges are accumulated in a counter that will increment on each edge
     * of the echo (return) signal. If the count is not at least 2, then the
     ...
     *
     * When in Automatic mode, all sensors will fire in round robin, waiting a
     * set time between each sensor.
     *
     * @param enabling Set to true if round robin scheduling should start for
     * all the ultrasonic sensors. This scheduling method assures that the
     * sensors are non-interfering because no two sensors fire at the same time.
     * If another scheduling algorithm is preferred, it can be implemented by
     * pinging the sensors manually and waiting for the results to come back.
     */
     ...
     *
     * @return Range in inches of the target returned from the ultrasonic
     * sensor. If there is no valid value yet, i.e. at least one measurement
  ...
  private:
     ...
     * there are two digital I/O channels allocated. If the system was running
     * in automatic mode (round robin) when the new sensor is added, it is
     * stopped, the sensor is added, then automatic mode is restored.
     */
    void Initialize();
     ...
     * will change while it's running. Make sure to disable automatic mode
