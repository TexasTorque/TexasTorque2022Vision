...
class Counter : public ErrorBase,
  ...
  public:
     ...
     *
     * They all must be selected by calling functions to specify the upsource
     * and the downsource independently.
     *
     ...
     * other objects such as encoders or if the Digital Source is not a Digital
     * Input channel (such as an Analog Trigger).
     *
     * The counter will start counting immediately.
     ...
     * Set the up counting DigitalSource.
     *
     * @param source Reference to the DigitalSource object to set as the up
     * source
     */
    void SetUpSource(DigitalSource& source);
     ...
     *
     * @param source Pointer to the DigitalSource object to set as the down
     * source
     */
     ...
     *
     * This allows counters to change the direction they are counting in the
     * case of 1X and 2X quadrature encoding only. Any other counter mode isn't
     * supported.
     *
     ...
     * the current value. Next time it is read, it might have a different value.
     */
     ...
     * and posted to a register on the FPGA.  When the timer detects that the
     * event source has stopped (based on the MaxPeriod) the buffer of samples
     * to be averaged is emptied.  If you enable the update when empty, you will
     * be notified of the stopped source and the event time will report 0
     * samples. If you disable update when empty, the most recent average will
     * remain on the output until a new sample is acquired.  You will never see
     * 0 samples output (except when there have been no events since an FPGA
     * reset) and you will likely not see the stopped bit become true (since it
     * is updated at the end of an average and there are no samples to average).
     *
