...
class InterruptableSensorBase : public ErrorBase {
  public:
     ...
     *
     * Request interrupts in asynchronous mode where the user's interrupt
     * handler will be called when the interrupt fires. Users that want control
     * over the thread priority should use the synchronous method with their own
     * spawned thread. The default is interrupt on rising edges only.
     */
     ...
     *
     * Request interrupts in synchronous mode where the user program will have
     * to explicitly wait for the interrupt to occur using WaitForInterrupt. The
     ...
     *
     * This deallocates all the chipobject structures and disables any
     * interrupts.
     */
     ...
     *
     * You should <b>NOT</b> attempt to read the sensor from another thread
     ...
     *
     * Interrupts are disabled when the RequestInterrupt call is made. This
     * gives time to do the setup of the other options before starting to field
     * interrupts.
     */
