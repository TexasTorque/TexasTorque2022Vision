...
namespace {
    ...
    public:
        ...
        JacksPipeline(int index, nt::NetworkTableInstance& ntinst, Bound bounds) {  
             wpi::outs() << "Pipe: " << index << "\n";
...
int main(int argc, char *argv[]) {
    ...
    Bound color = alliance.GetBoolean(false) ? Bound(RED) : Bound(BLUE);

    JacksPipeline* pipe1 = new JacksPipeline(0, ntinst, color);
    std::thread([&] {
     ...
     JacksPipeline* pipe2 = new JacksPipeline(1, ntinst, color);
    std::thread([&] {
        frc::VisionRunner<JacksPipeline> runner(
                cameras[1], pipe2,
                [&](JacksPipeline &pipeline) {});
