...
std::vector<cv::Point>
getLargestContourByArea(std::vector<std::vector<cv::Point>> contours) {
    int id = -1;
...
void update(cv::Mat* input, cv::Mat* output, Alliance alliance) {
    ...
    for (const std::vector<cv::Point>& contour : contours) {
        ...
        for (int i = 0; i < 4; i++) {
            cv::line(*input, vertices[i], vertices[(i + 1) % 4], cv::Scalar(255,
    255, 255), 2);
        }

        //cv::putText(*input, getShapeString(detectShape(contour)),
        //        rect.center, cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255,
    255), 2);
